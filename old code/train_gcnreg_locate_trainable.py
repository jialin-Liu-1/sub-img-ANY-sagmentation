...
#from multi.GCN_REG_location import GraphEnhancedCoarseInfoExtractor
#from multi.GCN_REG_location_tranable import GraphEnhancedCoarseInfoExtractor
#from multi.GCNREG_trainable_light import GraphEnhancedCoarseInfoExtractor
#from multi.GCNREG_width_first import GraphEnhancedCoarseInfoExtractor
from multi.REG_only import GraphEnhancedCoarseInfoExtractor
...
class CoarseExtractorTrainer:
    ...
    def _create_model(self):
        ...
        """model = GraphEnhancedCoarseInfoExtractor(
            ...
            gcn_hidden_dim=192,
            gcn_out_dim=192,
            use_spatial_regressor = True,  # 启用空间回归器
            fusion_weight = 0.5
        ).to(self.device)"""

        model = GraphEnhancedCoarseInfoExtractor(
            image_size=(512, 512),
            base_channels=self.config.get('base_channels', 48),
            num_position_classes=6,
            num_width_classes=4,
            num_disentangled_features=6,
            dropout_rate=0.1,
    ...
    def train_epoch(self, model, train_loader, optimizer, epoch):
        ...
        for batch in train_bar:
            ...
            # 2. 生成多个增强版本，记录增强方法（每个batch内每张图像可能有不同增强方法）
            aug_data = []  # [(aug_outputs, aug_methods_list), ...]
            for _ in range(num_augmentations):
                images_aug, aug_methods = self.augmentation(images)  # aug_methods 是列表
            ...
            train_bar.set_postfix({
                ...
                'contrast': f'{loss_dict.get("contrast_raw", 0):.3f}'
            })
...
class CoarseExtractorTester:
    ...
    def _load_model(self, model_path):
        ...
        if 'model' in checkpoint:
            ...
            print(f"  base_channels: {train_config.get('base_channels', 48)}")

            model = GraphEnhancedCoarseInfoExtractor(
                image_size=(512, 512),
                base_channels=train_config.get('base_channels', 48),
                num_position_classes=6,
                num_width_classes=4,
                num_disentangled_features=6,
                dropout_rate=0.1,
    ...
    def _save_test_results(self, results, stats):
        ...
        with open(self.output_dir / 'test_report.txt', 'w', encoding='utf-8') as f:
            ...
            f.write("-" * 40 + "\n")
            f.write(f"  Height accuracy: {height_threshold_accuracy:.2%} ({stats['height_correct_threshold']}/{total})\n")
