...
class DSAAnalyzer:
    ...
    def process_sequence(self, image_frames, threshold_percent, frame_percent,
                         sequence_id, file_name, closing_iterations=1, kernel_size=3):
        """
        处理单个DSA序列的指定比例帧数，只标记像素值下降超过阈值的区域，
        并进行形态学后处理
        ...
        logger.info(f"    - 使用前 {frames_to_process}/{num_frames} 帧 ({frame_percent}%) 进行标记")
        ...
        for i in range(frames_to_process - 1):
            # 计算像素值下降（当前帧比下一帧亮，即对比剂流入导致信号降低）
            decrease = image_frames[i] - image_frames[i + 1]
            ...
            if decrease_count_val > 0:
                logger.debug(f"      - 帧 {i + 1}-{i + 2}: {decrease_count_val} 个下降像素点")
    ...
    def run(self, threshold_percent=30, frame_percent=50, closing_iterations=2, kernel_size=3):
        """
        运行分析流程 - 自动处理所有文件
        ...
        logger.info(f"使用前 {frame_percent}% 的帧进行标记")
...
def main():
    ...
    THRESHOLD_PERCENT = 23  # 范围: 0-100，可根据需要调整

    # 帧数使用比例设置 - 使用前百分之多少的帧进行血管标记
    FRAME_PERCENT = 65  # 使用前30%的帧进行标记（例如总帧数20帧，则使用前6帧）
    # 可以根据需要调整：
    # - 较小的值（如20-30%）：只使用早期的帧，可能只标记到主要血管
    # - 较大的值（如50-60%）：使用更多的帧，可能标记到更细的血管
    # - 100%：使用所有帧进行标记

    # 形态学后处理参数
    CLOSING_ITERATIONS = 4  # 闭运算迭代次数，设为0可关闭后处理
