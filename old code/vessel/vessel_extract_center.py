...
class DSAAnalyzer:
    ...
    def extract_vessel_centerline(self, binary_mask):
        ...
        Args:
            binary_mask: 输入的二值血管掩膜

        Returns:
            centerline: 血管中心线的二值图像 (0或1)
        """
        try:
            # 确保输入是二值图像
            ...
            # 统计中心线像素数量
            centerline_pixels = np.sum(centerline)
            vessel_pixels = np.sum(binary_mask)

            if vessel_pixels > 0:
                centerline_ratio = centerline_pixels / vessel_pixels
                logger.info(f"    - 中心线提取: {centerline_pixels} 像素, 占比: {centerline_ratio:.2%}")
            else:
                logger.warning("    - 血管区域为空，无法提取中心线")
    ...
    def create_centerline_overlay(self, original_mask, centerline, file_name, sequence_id):
        ...
        # 原始血管标记用绿色半透明显示 (RGB: 0,255,0)
        # 将原始掩膜转换为绿色通道
        green_channel = (original_mask * 255).astype(np.uint8)
        overlay[:, :, 1] = green_channel  # 绿色通道

        # 中心线用红色显示 (RGB: 255,0,0)
        red_channel = (centerline * 255).astype(np.uint8)
        # 确保中心线覆盖在绿色上（红色通道优先级更高）
        overlay[:, :, 0] = red_channel

        # 对于中心线位置，同时将绿色通道置0，使红色更明显
    ...
    def process_sequence(self, image_frames, threshold_percent, frame_percent,
        ...
        Args:
            ...
            closing_iterations: 闭运算迭代次数
            kernel_size: 结构元素大小
            extract_centerline: 是否提取血管中心线

        Returns:
            binary_mask: 处理后的二值掩膜图像
            centerline: 提取的中心线（如果extract_centerline=True）
        """
        ...
        # 提取血管中心线
        centerline = None
        if extract_centerline and np.sum(marked_pixels) > 100:  # 只在血管区域足够大时提取中心线
            logger.info("    - 提取血管中心线...")
            centerline = self.extract_vessel_centerline(marked_pixels)
            if np.sum(centerline) > 0:
                self.stats['centerline_generated'] += 1
                logger.info(f"    - 中心线提取完成: {np.sum(centerline)} 像素")
            else:
                logger.warning("    - 中心线提取结果为空")
        elif extract_centerline:
            logger.warning("    - 血管区域太小，跳过中心线提取")
    ...
    def save_result(self, mask, centerline, file_name, sequence_id):
        ...
        if centerline is not None and np.sum(centerline) > 0:
            ...
            # 可选：同时保存单独的中心线图像（用于调试）
            centerline_only_filename = f"{file_name}_{sequence_id}_centerline_only.png"
            centerline_only_path = os.path.join(self.centerline_output_dir, centerline_only_filename)
            centerline_only = (centerline * 255).astype(np.uint8)
            centerline_only_img = Image.fromarray(centerline_only)
            centerline_only_img.save(centerline_only_path)

        return output_path

    def run(self, threshold_percent=30, frame_percent=50, gaussian_sigma=1.0,
            closing_iterations=2, kernel_size=3, extract_centerline=True):
        """
        运行分析流程 - 自动处理所有文件
        ...
        logger.info(f"中心线叠加输出目录: {self.centerline_output_dir}")
        logger.info("=" * 70)
...
def main():
    ...
    OUTPUT_DIR = r"D:\med_data\ai\vessel"  # 血管标记图输出目录
    CENTERLINE_OUTPUT_DIR = r"D:\med_data\ai\vessel_center"  # 中心线叠加图输出目录

    # 阈值设置 - 只标记像素值下降超过此阈值的区域
    THRESHOLD_PERCENT = 20  # 范围: 0-100，可根据需要调整
    ...
    # 形态学后处理参数
    CLOSING_ITERATIONS = 3  # 闭运算迭代次数，设为0可关闭后处理
    KERNEL_SIZE = 3  # 结构元素大小（必须是奇数）
