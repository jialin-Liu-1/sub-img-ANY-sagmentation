...
import pickle
import shutil
from multi.locate_net_feature import ImprovedCoarseInfoExtractor  # 导入新模型
...
class CoarseExtractorTrainer:
    ...
    def train(self, cache_session: str):
        ...
        for epoch in range(self.config['num_epochs']):
            ...
            print(f"    Contrast Loss: {train_metrics.get('contrast', 0):.4f}")
        ...
        return final_complete_model_path  # 返回完整模型的路径
...
class CoarseExtractorTester:
    ...
    def test_from_cache(self):
        ...
        for batch_idx, batch in enumerate(tqdm(val_loader, desc="Testing progress")):
            # 直接从缓存加载数据（预处理已经和训练时完全一致）
            images = batch['image'].to(self.device)
            positions = batch['position'].to(self.device)

            height_true = batch['height_ratio'].item()
            width_true = batch['width_ratio_true'].item()
            width_scaled_true = batch['width_ratio_scaled'].item()
            size_class_true = batch['size_class'].item()
            ...
            with torch.no_grad():
                ...
                width_pred_scaled = outputs['width_value'].item()
                width_pred_true = width_pred_scaled / 2.5
