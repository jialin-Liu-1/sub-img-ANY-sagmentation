...
class AttentionMaskGenerator(nn.Module):
    ...
    def __init__(self, image_size: Tuple[int, int] = (512, 512), min_radius_ratio: float = 0.06):
        ...
        # 定义尺寸分类对应的窗口高度比例（真实值范围0-0.4）
        # 这些比例决定了注意力条带的高度
        self.class_to_width_ratio = {
            0: 0.04,  # tiny: 小窗口
            1: 0.08,  # small: 中等窗口
            2: 0.19,  # medium: 较大窗口
            3: 0.40  # large: 大窗口
        ...
        print(f"尺寸分类对应的宽度比例: {self.class_to_width_ratio}")
    ...
    def _get_width_ratio_from_class(self, size_class: torch.Tensor) -> torch.Tensor:
        """
        将尺寸分类转换为宽度比例

        Args:
            size_class: [B] 尺寸分类 (0,1,2,3)

        Returns:
            width_ratio: [B] 宽度比例，范围0-0.4
        """
        B = size_class.shape[0]
        width_ratios = torch.zeros(B, device=self.device)

        for b in range(B):
            class_id = size_class[b].item()
            width_ratios[b] = self.class_to_width_ratio.get(class_id, self.min_radius_ratio)

        return width_ratios

    def forward(self, height_ratio: torch.Tensor,
                size_class: torch.Tensor = None,
                width_ratio: torch.Tensor = None) -> torch.Tensor:
        """
        生成注意力掩膜

        Args:
            height_ratio: [B] 高度比例，范围0-1，表示动脉瘤中心在图像中的高度位置
            size_class: [B] 尺寸分类 (0,1,2,3) - 优先使用
            width_ratio: [B] 宽度比例，范围0-0.4 - 备用（当size_class为None时使用）

        Returns:
            attention_mask: [B, 1, H, W] 注意力掩膜，条带形状
        """
        ...
        if size_class is not None:
            width_ratios = self._get_width_ratio_from_class(size_class)
        elif width_ratio is not None:
            width_ratios = width_ratio.clone()
        else:
            raise ValueError("Either size_class or width_ratio must be provided")

        batch_masks = []

        for b in range(B):
            h_ratio = height_ratio[b].item()
            w_ratio = width_ratios[b].item()
...
class CoarseExtractorTrainer:
    ...
    def _create_model(self):
        ...
        model = GraphEnhancedCoarseInfoExtractor(
            image_size=(512, 512),
            # 高度编码器配置
            height_encoder_channels=self.config.get('height_encoder_channels', 32),
            height_encoder_blocks=self.config.get('height_encoder_blocks', 2),
            # 宽度编码器配置（轻量化）
            width_encoder_channels=self.config.get('width_encoder_channels', 24),
            width_encoder_blocks=self.config.get('width_encoder_blocks', 1),
            # 通用配置
            ...
            use_soft_label=True,
            use_spatial_regressor=True,
            use_auxiliary_heads=self.config.get('use_auxiliary_heads', True),
            gcn_hidden_dim=self.config.get('gcn_hidden_dim', 128),
            gcn_out_dim=self.config.get('gcn_out_dim', 128)
        ).to(self.device)

        # 打印模型参数分析
        total_params = 0
        print("\n" + "=" * 60)
        print("Model Parameter Analysis (Separated Pathways)")
        print("=" * 60)

        # 分别统计各部分的参数量
        height_params = sum(p.numel() for p in model.height_encoder.parameters())
        width_params = sum(p.numel() for p in model.width_encoder.parameters())
        gcn_params = sum(p.numel() for p in model.width_gcn.parameters())
        height_head_params = sum(p.numel() for p in model.height_head.parameters()) + \
                             sum(p.numel() for p in model.feature_compressor.parameters()) + \
                             sum(p.numel() for p in model.position_encoder.parameters()) + \
                             sum(p.numel() for p in model.fusion_layer.parameters())
        spatial_params = sum(p.numel() for p in model.spatial_regressor.parameters()) if model.spatial_regressor else 0
        aux_params = sum(p.numel() for p in model.feature_disentangler.parameters()) + \
                     sum(p.numel() for p in model.aneurysm_analyzer.parameters()) if model.use_auxiliary_heads else 0

        print(f"  Height Encoder: {height_params / 1e6:.2f}M ({height_params:,})")
        print(f"  Width Encoder: {width_params / 1e6:.2f}M ({width_params:,})")
        print(f"  GCN Module: {gcn_params / 1e6:.2f}M ({gcn_params:,})")
        print(f"  Height Head: {height_head_params / 1e6:.2f}M ({height_head_params:,})")
        if spatial_params > 0:
            print(f"  Spatial Regressor: {spatial_params / 1e6:.2f}M ({spatial_params:,})")
        if aux_params > 0:
            print(f"  Auxiliary Heads: {aux_params / 1e6:.2f}M ({aux_params:,})")

        total_params = height_params + width_params + gcn_params + height_head_params + spatial_params + aux_params
        print("-" * 60)
    ...
    def train_epoch(self, model, train_loader, optimizer_height, optimizer_width, optimizer_aux, epoch):
        """训练一个epoch - 统一反向传播，分别更新"""
        model.train()
        self.criterion.update_epoch(epoch)

        num_augmentations = self.config.get('num_contrast_views', 4)

        # 渐进式增强
        if epoch < 6:
            num_augmentations = min(num_augmentations, 2)
        elif epoch < 12:
        ...
        train_accuracy = 0
        train_spatial_confidence = 0
        train_regresser_weights = 0
        ...
        for batch in train_bar:
            ...
            # 1. 原始图像前向
            outputs_orig = model(images, positions)

            # 2. 生成多个增强版本
            aug_data = []
            for _ in range(num_augmentations):
                images_aug, aug_methods = self.augmentation(images)
                aug_outputs = model(images_aug, positions)
                aug_data.append((aug_outputs, aug_methods))

            # 3. 计算损失（返回 tensor 的 total_loss）
            loss, loss_dict = self.criterion(outputs_orig, targets, aug_data)

            # ========== 4. 统一反向传播，分别更新 ==========
            # 4.1 清零所有优化器的梯度
            optimizer_height.zero_grad()
            optimizer_width.zero_grad()
            if optimizer_aux:
                optimizer_aux.zero_grad()

            # 4.2 反向传播一次（计算所有参数的梯度）
            loss.backward()

            # 4.3 梯度裁剪（分别处理不同参数组）
            torch.nn.utils.clip_grad_norm_(model.height_encoder.parameters(), max_norm=1.0)
            torch.nn.utils.clip_grad_norm_(model.spatial_regressor.parameters(), max_norm=1.0)
            torch.nn.utils.clip_grad_norm_(model.feature_compressor.parameters(), max_norm=1.0)
            torch.nn.utils.clip_grad_norm_(model.position_encoder.parameters(), max_norm=1.0)
            torch.nn.utils.clip_grad_norm_(model.fusion_layer.parameters(), max_norm=1.0)
            torch.nn.utils.clip_grad_norm_(model.height_head.parameters(), max_norm=1.0)
            torch.nn.utils.clip_grad_norm_(model.vessel_verifier.parameters(), max_norm=1.0)
            torch.nn.utils.clip_grad_norm_(model.attention_generator.parameters(), max_norm=1.0)

            torch.nn.utils.clip_grad_norm_(model.width_encoder.parameters(), max_norm=1.0)
            torch.nn.utils.clip_grad_norm_(model.width_gcn.parameters(), max_norm=1.0)
            torch.nn.utils.clip_grad_norm_(model.width_head.parameters(), max_norm=1.0)

            if model.use_auxiliary_heads:
                torch.nn.utils.clip_grad_norm_(model.feature_disentangler.parameters(), max_norm=1.0)
                torch.nn.utils.clip_grad_norm_(model.aneurysm_analyzer.parameters(), max_norm=1.0)

            # 4.4 分别更新参数（每个优化器只更新自己管理的参数）
            optimizer_height.step()
            ...
            train_height_loss += loss_dict.get('height_raw', 0)
            train_width_reg_loss += loss_dict.get('width_reg_raw', 0)
            train_width_cls_loss += loss_dict.get('width_cls_raw', 0)
            ...
            # 记录监控指标
            if 'spatial_confidence' in outputs_orig and outputs_orig['spatial_confidence'] is not None:
                train_spatial_confidence += outputs_orig['spatial_confidence'].mean().item()
            if 'regresser_weights' in outputs_orig:
                train_regresser_weights += outputs_orig['regresser_weights'].mean().item()

            # 更新进度条
            train_bar.set_postfix({
                'loss': f'{loss.item():.3f}',
                'acc': f'{loss_dict["accuracy"]:.2%}'
                #'conf': f'{outputs_orig.get("spatial_confidence", torch.tensor(0)).mean().item():.2f}'
                #'w': f'{outputs_orig.get("regresser_weights", torch.tensor(0)).mean().item():.2f}'
            })
        ...
        result = {
            ...
            'spatial_confidence': train_spatial_confidence / num_batches if num_batches > 0 else 0,
            'regresser_weights': train_regresser_weights / num_batches if num_batches > 0 else 0
        }
    ...
    def _compute_height_loss(self, pred, target):
        ...
        abs_error = torch.abs(pred - target)
        mse = (pred - target) ** 2
        focal_weight = (abs_error + 1e-8) ** self.criterion.focal_gamma
        height_focal_loss = (focal_weight * mse).mean()
        height_l1_loss = self.criterion.l1_loss(pred, target)
        return (1 - self.criterion.focal_mix) * height_focal_loss + self.criterion.focal_mix * height_l1_loss

    def _compute_width_reg_loss(self, pred, target):
        """计算宽度回归损失"""
        return self.criterion.mse_loss(pred, target)

    def _compute_width_cls_loss(self, outputs, targets):
        """计算宽度分类损失"""
        if 'width_logits' in outputs:
            return self.criterion.ce_loss(outputs['width_logits'], targets['size_class'])
        else:
            pred_width_true = outputs['width_value'] / 2.5
            centers = torch.tensor([0.05, 0.15, 0.3375, 0.7375], device=self.device)
            distances = torch.abs(pred_width_true.unsqueeze(1) - centers.unsqueeze(0))
            logits = -distances / 0.1
            return self.criterion.ce_loss(logits, targets['size_class'])
    ...
    def _compute_accuracy(self, outputs, targets):
        ...
        if 'width_logits' in outputs:
            ...
            pred_classes[(pred_width_true >= 0.2) & (pred_width_true < 0.475)] = 2
            pred_classes[pred_width_true >= 0.475] = 3
        return (pred_classes == targets['size_class']).float().mean().item()
    ...
    def train(self, cache_session: str):
        """训练模型 - 统一反向传播，分别更新"""
        print("\n" + "=" * 60)
        print("Starting Separated Pathways Model Training")
        print("=" * 60)
        ...
        # ========== 创建双优化器（管理不同参数组） ==========
        # 高度路径参数
        height_params = list(model.height_encoder.parameters()) + \
                        list(model.spatial_regressor.parameters()) + \
                        list(model.feature_compressor.parameters()) + \
                        list(model.position_encoder.parameters()) + \
                        list(model.fusion_layer.parameters()) + \
                        list(model.height_head.parameters()) + \
                        list(model.vessel_verifier.parameters()) + \
                        list(model.attention_generator.parameters())

        # 宽度路径参数
        width_params = list(model.width_encoder.parameters()) + \
                       list(model.width_gcn.parameters()) + \
                       list(model.width_head.parameters())

        # 辅助任务参数（如果使用）
        if model.use_auxiliary_heads:
            aux_params = list(model.feature_disentangler.parameters()) + \
                         list(model.aneurysm_analyzer.parameters())
        else:
            aux_params = []

        # 创建优化器
        optimizer_height = torch.optim.AdamW(
            height_params,
            lr=self.config.get('height_lr', 1e-4),
            weight_decay=self.config.get('weight_decay', 1e-4),
            betas=(0.9, 0.999)
        )

        optimizer_width = torch.optim.AdamW(
            width_params,
            lr=self.config.get('width_lr', 1e-4),
            weight_decay=self.config.get('weight_decay', 1e-4),
            betas=(0.9, 0.999)
        )
        ...
        if aux_params:
            optimizer_aux = torch.optim.AdamW(
                aux_params,
                lr=self.config.get('aux_lr', 1e-4),
                weight_decay=self.config.get('weight_decay', 1e-4),
                betas=(0.9, 0.999)
            )
        else:
        ...
        # 学习率调度器
        scheduler_height = torch.optim.lr_scheduler.ReduceLROnPlateau(
            optimizer_height, mode='min', factor=0.5, patience=5, min_lr=1e-6
        )
        scheduler_width = torch.optim.lr_scheduler.ReduceLROnPlateau(
            optimizer_width, mode='min', factor=0.5, patience=5, min_lr=1e-6
        )
        scheduler_aux = torch.optim.lr_scheduler.ReduceLROnPlateau(
            optimizer_aux, mode='min', factor=0.5, patience=5, min_lr=1e-6
        ) if optimizer_aux else None
        ...
        for epoch in range(self.config['num_epochs']):
            ...
            # 训练一个epoch（使用统一反向传播版本）
            train_metrics = self.train_epoch(
                model, train_loader,
                optimizer_height, optimizer_width, optimizer_aux,
                epoch
            )
            ...
            # 更新学习率调度器
            scheduler_height.step(val_metrics['total'])
            scheduler_width.step(val_metrics['total'])
            if scheduler_aux:
                scheduler_aux.step(val_metrics['total'])

            current_lr_height = optimizer_height.param_groups[0]['lr']
            current_lr_width = optimizer_width.param_groups[0]['lr']
            ...
            self.history['learning_rate'].append(current_lr_height)

            # 打印统计信息
            print(f"\n  Train Total Loss: {train_metrics['total']:.4f}")
            ...
            if 'spatial_confidence' in train_metrics:
                print(f"    Spatial Confidence: {train_metrics['spatial_confidence']:.3f}")
            if 'regresser_weights' in train_metrics:
                print(f"    Regressor Weights: {train_metrics['regresser_weights']:.3f}")
            print(f"  Validation Total Loss: {val_metrics['total']:.4f}")
            print(f"    Validation Accuracy: {val_metrics['accuracy']:.2%}")
            print(f"  Learning Rates: Height={current_lr_height:.6f}, Width={current_lr_width:.6f}")
            ...
            if val_metrics['total'] < best_val_loss:
                ...
                torch.save({
                    'epoch': epoch + 1,
                    'model_state_dict': model.state_dict(),
                    'optimizer_height_state_dict': optimizer_height.state_dict(),
                    'optimizer_width_state_dict': optimizer_width.state_dict(),
                    'train_metrics': train_metrics,
                    'val_metrics': val_metrics,
                    'config': self.config,
                }, best_model_path)
...
class CoarseExtractorTester:
    """改进版测试器 - 使用与训练一致的width_logits进行分类"""
    ...
    def __init__(self, model_path, config, cache_session=None, mask_dir=None):
        ...
        for dir_path in [self.comparison_dir, self.focused_dir, self.attention_dir]:
            dir_path.mkdir(exist_ok=True, parents=True)

        print(f"Test results will be saved to: {self.output_dir}")
        print(f"  - Comparison images: {self.comparison_dir}")
        print(f"  - Focused images: {self.focused_dir}")
        print(f"  - Attention maps: {self.attention_dir}")
    ...
    def _load_model(self, model_path):
        ...
        if 'model' in checkpoint:
            ...
            model = GraphEnhancedCoarseInfoExtractor(
                image_size=(512, 512),
                height_encoder_channels=train_config.get('height_encoder_channels', 32),
                height_encoder_blocks=train_config.get('height_encoder_blocks', 2),
                width_encoder_channels=train_config.get('width_encoder_channels', 24),
                width_encoder_blocks=train_config.get('width_encoder_blocks', 1),
                num_position_classes=6,
                num_width_classes=4,
                num_disentangled_features=6,
                dropout_rate=train_config.get('dropout_rate', 0.1),
                use_soft_label=True,
                use_spatial_regressor=train_config.get('use_spatial_regressor', True),
                use_auxiliary_heads=train_config.get('use_auxiliary_heads', False),
                gcn_hidden_dim=train_config.get('gcn_hidden_dim', 128),
                gcn_out_dim=train_config.get('gcn_out_dim', 128)
            ).to(self.device)

            # 使用strict=True确保所有权重都被正确加载
            missing_keys, unexpected_keys = model.load_state_dict(
                checkpoint['model_state_dict'],
                strict=True  # 改为True，确保权重完全匹配
            )
    ...
    def _load_aneurysm_mask(self, filename: str) -> Optional[np.ndarray]:
        ...
        try:
            basename = os.path.splitext(filename)[0]
            possible_names = [
                f"{basename}.tif", f"{basename}.tiff", f"{basename}.png",
                f"{basename}.jpg", f"{basename}_mask.tif", f"{basename}_mask.png"
            ]
    ...
    def test_from_cache(self):
        """从缓存会话中读取验证集数据进行测试 - 使用模型直接输出的width_class"""
        ...
        print(f"  width_class: {outputs['width_class'][0].item()}")  # 直接使用模型输出的分类
        if 'width_logits' in outputs:
            print(f"  width_logits (from regression): {outputs['width_logits'][0].tolist()}")
        ...
        for batch_idx, batch in enumerate(tqdm(val_loader, desc="Testing progress")):
            ...
            with torch.no_grad():
                outputs = self.model(images, positions)
                height_preds = outputs['height_ratio']
                width_preds_scaled = outputs['width_value']
                width_preds_true = width_preds_scaled / 2.5
                attention_maps = outputs.get('attention_map', None)

                # 【关键改进】直接使用模型输出的 width_class
                if 'width_class' in outputs:
                    pred_classes = outputs['width_class']  # [batch_size] 直接使用！
                else:
                    # 备用方案：从 width_logits 计算
                    print(f"⚠️ Warning: width_class not found, using width_logits.argmax()")
                    pred_classes = outputs['width_logits'].argmax(dim=1)

                # 【批处理生成注意力掩膜】
                # 预测掩膜：使用模型预测的高度 + 模型预测的分类
                attention_masks_pred = self.mask_generator(
                    outputs['height_ratio'],  # 模型预测的高度
                    size_class=pred_classes  # 模型预测的分类（直接输出）
                )  # shape: [batch_size, 1, H, W]

                # 真实掩膜：使用真实标签（仅用于对比）
                gt_classes = batch['size_class'].to(self.device)
                gt_heights = batch['height_ratio'].to(self.device)
                attention_masks_gt = self.mask_generator(
                    gt_heights,
                    size_class=gt_classes
                )  # shape: [batch_size, 1, H, W]
            ...
            for i in range(batch_size):
                ...
                # 获取模型预测值
                height_pred = height_preds[i].item()
                pred_class = pred_classes[i].item()  # 直接使用模型输出的分类
                ...
                # 计算误差
                height_error = abs(height_pred - height_true)
                width_error = abs(width_preds_true[i].item() - width_true)

                height_correct = height_error < self.error_threshold
                width_correct = width_error < self.error_threshold
                both_correct = height_correct and width_correct
                ...
                # 获取注意力掩膜
                image_np = images[i, 0].cpu().numpy()
                attention_mask_pred_np = attention_masks_pred[i, 0].cpu().numpy()
                attention_mask_gt_np = attention_masks_gt[i, 0].cpu().numpy()
                ...
                save_image(gt_focused, self.focused_dir / f"{basename}_gt_focused.png")

                # 保存注意力图（模型生成的attention_map）
                if attention_maps is not None:
                    att_map = attention_maps[i, 0].cpu().numpy()
                    att_map_normalized = (att_map - att_map.min()) / (att_map.max() - att_map.min() + 1e-8)
                    save_image(att_map_normalized, self.attention_dir / f"{basename}_attention_map.png")
                ...
                results.append({
                    'filename': filename,
                    'height_pred': height_pred,
                    'width_pred': width_preds_true[i].item(),
                    'height_true': height_true,
                    'width_true': width_true,
                    'height_error': height_error,
                    'width_error': width_error,
    ...
    def _create_comparison_plot(self, basename, image, pred_mask, gt_mask,
                                height_pred, width_pred, height_true, width_true,
                                position_name, size_category, size_correct,
                                pred_class_name, true_class_name,
                                height_correct, width_correct):
        """创建对比图"""
        height_status = f"✓" if height_correct else f"✗"
        width_status = f"✓" if width_correct else f"✗"

        images_to_plot = [image, pred_mask, gt_mask]
        titles = [
            'Original Image',
            f'Predicted Mask\nH: {height_pred:.3f} ({height_status})\nW: {width_pred:.3f} ({width_status})',
            f'Ground Truth\nH: {height_true:.3f}\nW: {width_true:.3f}'
        ]

        suptitle = (f"{basename}\nType: {position_name} | Size: {size_category}\n"
                    f"Pred: {pred_class_name} vs GT: {true_class_name} - {'✓' if size_correct else '✗'}")
    ...
    def _save_test_results(self, results, stats):
        ...
        with open(self.output_dir / 'test_report.txt', 'w', encoding='utf-8') as f:
            ...
            f.write("2. Size Classification Accuracy (using width_logits)\n")
            f.write("-" * 40 + "\n")
        ...
        print(
            f"\nSize classification accuracy (using width_logits): {size_accuracy:.2%} ({stats['size_correct']}/{total})")
...
def main():
    ...
    config = {
        ...
        'val_image_dir': "D:/med_data/ai/translate/reverse/test",
        ...
        # ========== 模型参数 ==========
        'image_size': (512, 512),
        'height_encoder_channels': 48,
        'height_encoder_blocks': 2,
        'width_encoder_channels': 32,
        'width_encoder_blocks': 1,
        'num_disentangled_features': 6,
        'dropout_rate': 0.1,
        'use_soft_label': True,

        # ========== 损失函数权重 ==========
        'height_weight': 4,           # 高度损失权重
        'width_reg_weight': 1.0,        # 宽度回归损失权重
        'width_cls_weight': 1.0,        # 宽度分类损失权重

        # Focal Loss参数
        'use_focal': True,
        'focal_gamma': 2.0,
        'focal_mix': 0.6,               # L1占比70%，Focal占30%

        # 空间位置监督
        'spatial_weight': 0.5,

        # 辅助任务
        'contrast_weight': 0.15,        # 对比学习损失权重
        'orthogonal_weight': 0.1,       # 正交性损失权重
        'aneurysm_weight': 0.15,        # 动脉瘤特征一致性损失权重
        'contrast_temperature': 0.2,    # 对比学习温度参数

        # ========== 对比学习增强参数 ==========
        'num_contrast_views': 3,        # 每个样本生成的增强版本数（方案A）

        # ========== 训练参数 ==========
        'batch_size': 8,
        'num_epochs': 70,
        'height_lr': 1e-4,
        'width_lr': 1e-4,  # 宽度任务用更大学习率
        'aux_lr': 1e-4,
        'weight_decay': 1e-4,
        ...
        'model_save_root': "D:/med_data/ai/loc_seperate",
