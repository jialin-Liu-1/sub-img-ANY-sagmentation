...
class DSAAnalyzer:
    ...
    def create_minip_projection(self, image_frames):
        """
        创建最小值投影图像（MinIP）
        对多帧图像沿时间轴取最小值，显示造影剂最浓的时刻
        """
        if image_frames is None or len(image_frames) == 0:
            return None

        # 沿时间轴（第0维）取最小值
        minip = np.min(image_frames, axis=0)
        return minip
    ...
    def save_aneurysm_related_results(self, circle_mask, original_mask, longest_centerline,
        ...
        if circle_mask is not None:
            ...
            # 4. 保存DSA图像（使用最小值投影MinIP）
            if image_frames is not None and len(image_frames) > 0:
                # 创建最小值投影图像
                minip_frame = self.create_minip_projection(image_frames)

                if minip_frame is not None:
                    # 归一化到0-255
                    minip_min = minip_frame.min()
                    minip_max = minip_frame.max()
                    if minip_max > minip_min:
                        minip_frame = ((minip_frame - minip_min) / (minip_max - minip_min + 1e-6) * 255).astype(
                            np.uint8)
                    else:
                        minip_frame = np.zeros_like(minip_frame, dtype=np.uint8)

                    # 转换为彩色图像
                    dsa_with_mask = cv2.cvtColor(minip_frame, cv2.COLOR_GRAY2BGR)
                    ...
                    logger.info(f"    - DSA图像(MinIP)已保存: {dsa_path}")
