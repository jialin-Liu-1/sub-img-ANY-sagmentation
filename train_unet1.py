...
class DualChannelDataMatcher:
    ...
    def find_paired_data(self) -> List[Tuple[Path, Path, Path]]:
        ...
        for file_path in self.dicom_dir.iterdir():
            if file_path.is_file() and file_path.suffix.lower() not in ['.png', '.jpg', '.jpeg', '.tif', '.npz']:
                dicom_files.append(file_path)
        print(f"找到 {len(dicom_files)} 个DICOM文件")

        # 获取所有注意力图（PNG格式，以_att结尾或直接在gray文件夹中）
        attention_files = []
        for ext in ['*.png', '*.jpg', '*.jpeg']:
            attention_files.extend(list(self.attention_dir.glob(ext)))
        ...
        for att_path in attention_files:
            stem = att_path.stem
            # 移除可能的后缀
            for suffix in ['_att', '_attention', '_att_gray', '_att_hot']:
...
class StreamDataManager:
    ...
    def __init__(self, cache_root: str):
        ...
        print(f"缓存目录: {self.cache_root}")

    def create_cache_session(self) -> str:
        """创建新的缓存会话"""
...
class DualChannelDataPreprocessor:
    ...
    def process_and_save(self, stream_manager: StreamDataManager,
                         val_ratio: float = 0.1,
        ...
        output_session = stream_manager.create_cache_session()
        output_path = Path(output_session)

        # 保存各数据集
        self._save_split(train_data, output_path / "train", "训练集", stream_manager)
        self._save_split(val_data, output_path / "val", "验证集", stream_manager)
        self._save_split(test_data, output_path / "test", "测试集", stream_manager)
    ...
    def _save_split(self, data_list: List, save_dir: Path, desc: str, stream_manager: StreamDataManager):
        """保存数据划分"""
        ...
        for dicom_path, att_path, mask_path in tqdm(data_list, desc=desc):
            ...
            stream_manager.save_sample(sample_data, str(save_path))
...
class DualChannelUNet(nn.Module):
    ...
    def __init__(self, base_channels=32, dropout_rate=0.1, negative_slope=0.01):
        super().__init__()
...
class DualChannelSegmentationTrainer:
    ...
    def __init__(self, config):
        ...
        # 创建输出目录
        self.model_save_root = Path(config['model_save_root'])
        self.model_save_root.mkdir(parents=True, exist_ok=True)

        date_str = datetime.now().strftime('%Y%m%d')
        random_num = random.randint(100, 999)
        model_dir_name = f"dual_channel_{date_str}_{random_num}"

        self.output_dir = self.model_save_root / model_dir_name
    ...
    def train_epoch(self, model, train_loader, optimizer, epoch, total_epochs):
        """训练一个epoch"""
        model.train()
        total_loss = 0
        total_dice_loss = 0
    ...
    def validate(self, model, val_loader):
        ...
        with torch.no_grad():
            for batch in val_bar:
                ...
                # 实时更新进度条
                avg_dice = batch_dice / batch_valid if batch_valid > 0 else 0
                val_bar.set_postfix({
                    'loss': f'{loss.item():.4f}',
                    'dice': f'{avg_dice:.4f}'
                })
    ...
    def train(self, cache_session: str):
        ...
        for epoch in range(self.config['num_epochs']):
            train_metrics = self.train_epoch(model, train_loader, optimizer, epoch, self.config['num_epochs'])
            ...
            print(
                f"\n  Train - Loss: {train_metrics['loss']:.4f} | Dice: {train_metrics['dice_loss']:.4f} | L1: {train_metrics['l1_loss']:.4f}")
            print(
                f"  Val   - Loss: {val_metrics['loss']:.4f} | Dice: {val_metrics['dice_loss']:.4f} | L1: {val_metrics['l1_loss']:.4f}")
            print(
                f"  Val Metrics - Dice: {val_metrics['dice']:.4f} | IoU: {val_metrics['iou']:.4f} | LR: {current_lr:.6f}")
...
class DualChannelSegmentationTester:
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(tqdm(test_loader, desc="测试")):
                ...
                for i in range(images_np.shape[0]):
                    ...
                    if batch_idx * test_loader.batch_size + i < 400:
    ...
    def save_comparison_image(self, dsa_img, att_img, true_mask, pred_mask,
        ...
        try:
            ...
            axes[0, 3].set_title('predicted Mask(probability')
            axes[0, 3].axis('off')
            ...
            axes[1, 1].set_title('DSA+attention overlap')
            axes[1, 1].axis('off')
            ...
            axes[1, 2].set_title('overlap(red: predicted, green: GT, yellow: overlap)')
            axes[1, 2].axis('off')
            ...
            plt.suptitle(f'Segmentation result - {dicom_name}', fontsize=14)
            plt.tight_layout()
...
def main():
    ...
    config = {
        ...
        'attention_dir': "D:/med_data/multi/ATT_B",
    ...
    print(f"缓存目录: {config['cache_root']}")
