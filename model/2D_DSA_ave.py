...
def process_individual_dsa_files(input_folder, output_dicom_folder, output_png_folder=None,
    ...
    logger.info(f"开始逐个处理患者DSA图像（截取前 {time_fraction * 100:.0f}% 时间点）...")
    ...
    for filename in tqdm(dicom_files, desc="处理患者文件"):
        ...
        try:
            ...
            if len(pixel_array.shape) == 3:
                ...
                # 沿时间维度取平均值
                mean_projection = np.mean(selected_frames, axis=0).astype(pixel_array.dtype)
                ...
                # 修改投影类型描述
                projection_type = f"前{time_fraction * 100:.0f}%时间平均值投影"

                # 保存为DICOM格式
                if save_as_dicom(mean_projection, ds, output_dicom_path, projection_type):
...
if __name__ == "__main__":
    ...
    output_dicom_folder = r"D:\med_data\ANY\process\NEW_DSA"
    ...
    try:
        # 逐个处理每个患者的DSA文件（使用前50%时间点）
        process_individual_dsa_files(input_folder, output_dicom_folder,
